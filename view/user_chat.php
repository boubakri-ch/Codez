<?php 
  ...
  $login=new loginOps();
  $liste=$login->selectcompte_chat($unique_id);
...
<body>
    ...
    <section class="users">
      <header>
        <div class="content">
        <?php foreach($liste_2 as $row3)
        { ?>
          <div class="details">
            <span><?php echo $row3['fname']. " " . $row3['lname'] ?></span>
            <p><?php echo $row3['statut']; ?></p>
          </div>
        </div>
        <a href="logout.php?logout_id=<?php echo $row3['unique_id']; ?>" class="logout">Logout</a>
        <?php } ?>
      </header>
      <div class="search">
